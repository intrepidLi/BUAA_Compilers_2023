...
public class Function extends Constant{
    ...
    public ArrayList<BasicBlock> getBasicBlocks() {
        if (functionType == 0) {
            return functionInt.getBasicBlocks();
        } else {
            return functionVoid.getBasicBlocks();
        }
    }
    ...
    public Function(String name, Type type, boolean isBuiltIn, Type... paramType) {
        ...
        if (type instanceof IntegerType) {
            functionInt = new FunctionInt(name, isBuiltIn, type, this, paramType);
            functionType = 0;
        } else {
            functionVoid = new FunctionVoid(name, isBuiltIn, this, paramType);
            functionType = 1;
        }
    }
    ...
    public int getArgsNum() {
        if (functionType == 0) {
            return functionInt.getArgsNum();
        } else {
            return functionVoid.getArgsNum();
        }
    }
    ...
    public ArrayList<Type> getParamsType () {
        if (functionType == 0) {
            return functionInt.getParamsType();
        } else {
            return functionVoid.getParamsType();
        }
    }
}
